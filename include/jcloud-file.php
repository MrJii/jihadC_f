...
class jihadC {
	...
	public function init() {
		...
		if (isset($_GET['upload']) && $_GET['upload'] == '1' && jihadC::checkPermissions('ru')){
			$this->ajaxUpload();
		}
		
		// test if signup
		if (isset($_GET['signup']) && $_GET['signup'] == '1' && jihadCconf::get('allow_signup')){
			$this->initSignup();
			die;
		}
		
		// use auth?
		if (jihadCconf::get('use_auth') == true){
			$this->authenticate();
		}else{
			$_SESSION['simple_auth']['permissions'] = 'rwu';
			$_SESSION['simple_auth']['username'] = 'guest';
			$_SESSION['simple_auth']['cryptsalt'] = jihadCconf::get('encryption_salt');
		}
		...
		if ($_SESSION['simple_auth']['username'] == 'admin'){
			...
			$config_users = jihadC::getAllUsers();

			jihadC::display("header.php");
			jihadC::display("admin.php", $config_users);
			jihadC::display("footer.php");
			exit;
		}
		...
		$listing = jihadC::getDirectoryList();

		jihadC::display("header.php");
		jihadC::display("main.php", $listing);
		jihadC::display("footer.php");
	...
	public function initSignup() {
		...
		if (jihadCconf::get('signup_use_activation') && isset($_GET['activate'])){

			$key = $_GET['activate'];
			$user = jihadC::getUser($key, 'akey');

			if ($user){
				jihadC::updateUser($user['username'], array('akey' => '', 'permissions' => jihadCconf::get('default_permissions_after_activation')));
				$user['permissions'] = jihadCconf::get('default_permissions_after_activation');
				$this->loginUser($user);
			}

			header('Location: '.jihadCconf::get('base_url'));
			die;
		}
		...
		if (!empty($_POST)){
			...
			if (	!isset($username)
					...
					|| jihadC::getUser($username)
					|| jihadC::getUser($email, 'email')
					|| !isset($password)
					|| !isset($password2)
					|| !isset($_POST['captcha'])
					|| $password == ''
			...
			if ($errors == false){
				...
				if (jihadCconf::get('signup_use_activation')){

					$activationkey = sha1(mt_rand(10000,99999).time());
					$url = jihadCconf::get('base_url').'/?signup=1&activate='.$activationkey;

					$subject = jihadCconf::get('account_email_subject');
					$body = jihadCconf::get('account_email_text'). "\n\n" .$url;
				...
				$homedir = jihadCconf::get('repository').DS.$username;
				...
				// add new user
				jihadC::addUser($username, array(
				'password' => $password,
				'permissions' => jihadCconf::get('default_permissions_after_signup'),
				'homedir' => $homedir,
				'email' => $email,
				'akey' => $activationkey));

				// thanks on success or go activate!
				header('Location: '.jihadCconf::get('base_url').'/?signup=1&'.$reloadaction);
				die;
			}
		...
		if (isset($_GET['thanks'])){
			...
			jihadC::display("header.php");
			jihadC::display("signup.php", array('goactivate' => 1));

		}else{

			jihadC::display("header.php");
			jihadC::display("signup.php", array('errors' => $errors));
	...
	public function actions(){
		...
		// no read permissions?
		if (!jihadC::checkPermissions('r')){
			jihadC::writeLog('auth bad - no read access');
			jihadC::error(lang::get("Access Forbidden"));
			die;
		}
				...
				switch ($action){
					...
					case 'delete':
						...
						foreach ($_POST as $post_file){
							...
							if (in_array($post_file, jihadCconf::get('restricted_files'))) continue;
					...
					case 'rename':
						...
						if (jihadCconf::get('allow_rename_files') == false || !isset($_POST['oldname']) || !isset($_POST['newname'])) break;

						$oldname = $this->filterInput($this->decrypt($_POST['oldname']));
						$newname = $this->filterInput($_POST['newname']);

						if (in_array($oldname, jihadCconf::get('restricted_files')) || in_array($newname, jihadCconf::get('restricted_files'))) break;
					...
					case 'edit-save':
						...
						if (jihadCconf::get('allow_edit_files') == false || !isset($_POST['filename'])) break;

						$filename = $this->filterInput($this->decrypt($_POST['filename']));
						$content = $_POST['content'];

						if (in_array($filename, jihadCconf::get('restricted_files'))) break;

						file_put_contents($_SESSION['cwd'].DS.$filename, $content);

						jihadC::writeLog('edit file / save - '.$filename);
					...
					case 'simple-move':
						...
						if ($action == 'simple-copy'){
							$this->copyFiles($files, $_SESSION['cwd'], jihadCconf::get('repository').DS.$destination);
						}

						if($action == 'simple-move'){
							$this->moveFiles($files, $_SESSION['cwd'], jihadCconf::get('repository').DS.$destination);
						}
			...
			if (jihadC::checkPermissions('r')){
				switch ($action){
					case 'email':
						...
						if (jihadCconf::get('allow_email_links') != true || !isset($_POST['filelink']) || !isset($_POST['email'])) break;

						$to = $_POST['email'];
						$subject = jihadCconf::get('mail_link_subject');

						$link = filter_var($_POST['filelink'], FILTER_SANITIZE_STRING);

						$body = filter_var($_POST['email_content'], FILTER_SANITIZE_STRING);

						if (jihadCconf::get('use_googl_shorturl')){
		...
		if (isset($_GET['download']) && !empty($_GET['download'])){
			...
			if (in_array($filename, jihadCconf::get('restricted_files'))) die;
			...
			jihadC::writeLog('download - '.$filename);

			die;
		}

		// edit action - load file content via this ajax
		if (isset($_GET['edit-load']) && jihadC::checkPermissions('rw') && jihadCconf::get('allow_edit_files') == true){
			...
			jihadC::writeLog('edit file / load - '.$filename);

			die;
		}

		// new folder / new file
		if ((isset($_GET['newdir']) || isset($_GET['newfile'])) && jihadC::checkPermissions('rw')){
			...
			if (isset($_GET['newdir']) && $_GET['newdir'] != ''){
				...
				if (!in_array($newdir, jihadCconf::get('restricted_files')))
					mkdir($_SESSION['cwd'].DS.$newdir, jihadCconf::get('new_dir_mode'));

			} elseif (isset($_GET['newfile']) && $_GET['newfile'] != ''){
				$newfile = $this->filterInput($_GET['newfile']);

				if (!in_array($newfile, jihadCconf::get('restricted_files')))
					touch($_SESSION['cwd'].DS.$newfile);
			}

			jihadC::writeLog('create new - '.$newdir.$newfile);
		...
		if (isset($_GET['sortby']) || isset($_GET['sortinvert'])){
			...
			jihadC::writeLog('sort order '.$_SESSION['sort']['by']);
		...
		if (isset($_GET['tree']) || !empty($_GET['tree'])){
			...
			if ($tree_action == 'cd'){
				$dirs = $this->getDirectoryTree(jihadCconf::get('repository'), false, '?cd=jihadCloud');
			}

			if ($tree_action == 'copy' || $tree_action == 'move'){
				$dirs = $this->getDirectoryTree(jihadCconf::get('repository'), true, '');
			}

			echo $dirs;

			jihadC::writeLog('tree load');
			die;
		}

		// change password
		if (jihadCconf::get('allow_change_password') && isset($_POST['changepassword']) && !empty($_POST['changepassword'])){

			$new_password = rawurldecode($_POST['changepassword']);

			jihadC::updateUser($_SESSION['simple_auth']['username'], array('password' => $new_password));
	...
	public function adminActions(){
		...
		if (jihadCconf::get('mask_repository_path')) {
			$homedir = jihadCconf::get('repository').DS.$homedir;

			// do not allow dirs up to reposiroty
			if (strstr($homedir, jihadCconf::get('repository')) == false){
				$homedir = jihadCconf::get('repository');
			}
		}
		...
		if (isset($_POST['is_new']) && $_POST['is_new'] == 'yes'){
			jihadC::addUser($account, array('password' => $password, 'permissions' => $permissions, 'homedir' => $homedir, 'akey' => '', 'email' => $email));
			return;
		}

		// update user
		jihadC::updateUser($account, array('password' => $password, 'permissions' => $permissions, 'homedir' => $homedir, 'email' => $email));
	...
	public function changeDirectory(){
		...
		if (!isset($_SESSION['cwd']) || (isset($_GET['cd']) && $this->decrypt($_GET['cd']) == '')){
			$_SESSION['cwd'] = jihadCconf::get('repository');

			jihadC::writeLog('change dir - home');
		...
		if ($input && strpos($input, '..') === false){
			...
			$childDir = jihadCconf::get('repository').DS.$input;
			$childDir = str_replace('\\', '/', $childDir);

			// do not allow chdir outside reposiroty
			if (strstr($childDir, jihadCconf::get('repository')) == false){
				header('Location: '.jihadCconf::get('base_url'));
				die;
			}
			...
			if (is_dir($childDir)){
				...
				jihadC::writeLog('change dir - '.$input);
			}
	...
	public static function display($view, $params = null){
		...
		require_once jihadCconf::get('base_path')."/include/views/".$view;
	}
	...
	public function copyRecursively($source, $dest){
		...
		while (false !== $entry = $dir->read()) {
			...
			jihadC::writeLog('copy - '.$source.' > '.$dest);
		}
	...
	public function moveFiles($files, $source_dir, $destination_dir){
		...
		foreach($files as $file){
			...
			if ($file == '.' || $file == '..' || in_array($file, jihadCconf::get('restricted_files'))) return false;
	...
	public function renameFile($old_name, $new_name){
		...
		if (!file_exists($_SESSION['cwd'].DS.$new_name)){
			...
			jihadC::writeLog('rename - '.$old_name.' > '.$new_name);
	...
	public function zipFiles($input_files, $destination){
		...
		if (!jihadCconf::get('use_zip')) return;

		if (!extension_loaded('zip')) {
			jihadC::error('Zip PHP module is not installed on this server');
			die;
		}
		...
		if (!$zip->open($destination, ZIPARCHIVE::CREATE)) {
			jihadC::error('Archive could not be created');
		}
	...
	public function unzipFile($file){
		...
		if ($zip->open($file) === TRUE) {
			...
			for ($idx = 0; $idx < $zip->numFiles; $idx++) {
				...
				if (strpos(implode("", jihadCconf::get('restricted_files')), pathinfo($zname, PATHINFO_BASENAME)) !== false
			...
			jihadC::error("Unable to proccess file '{$file}'");
		}
	...
	public function ajaxUpload(){
		...
		require_once jihadCconf::get('base_path')."/include/blueimp/server/php/upload.class.php";
		...
		if (isset($_FILES['files']['name'])) {
			...
			if (in_array($filename, conf::get('restricted_files'))) die;
		}

		jihadC::writeLog('upload file '.$filename);
	...
	public function authenticate(){
		...
		if(isset($_SESSION['simple_auth']['username']) && $_SESSION['simple_auth']['username'] == 'guest' && jihadCconf::get('allow_guests') == false){
			session_destroy();
			session_start();
		}
		...
		if (jihadCconf::get('enable_password_recovery')
			...
			$user = jihadC::getUser($email, 'email');
				
			if ($user && filter_var($email, FILTER_VALIDATE_EMAIL)){

				$generatedKey = 'otp-'.sha1(mt_rand(10000,99999).time());
				jihadC::updateUser($user['username'], array('akey' => $generatedKey));

				$url = jihadCconf::get('base_url').'/?otp='.$generatedKey;
		...
		if (jihadCconf::get('enable_password_recovery')
			...
			$user = jihadC::getUser($otp, 'akey');
			
			if ($user){
				jihadC::updateUser($user['username'], array('akey' => ''));
		...
		if(!isset($_SESSION['simple_auth']['username']) || isset($_GET["login"])) {
			...
			if(isset($_POST["submit"])) {
				...
				$user = jihadC::getUser($_POST['username']);

				if (isset($user['permissions']) && !strstr($user['permissions'], 'r')
						|| ($user['username'] == 'guest' && jihadCconf::get('allow_guests') == false)){
					$errors = lang::get("Access Forbidden");
					jihadC::writeLog('auth bad - not activated');
				}

				if (!isset($_POST['username']) || !isset($_POST['password']) || $_POST['username'] == '' || $_POST['password'] == ''){
					$errors = lang::get("Enter username and password.");
					jihadC::writeLog('auth bad - blank fields');
				}
				...
				if(!$errors && $user['username'] == $_POST['username'] && jihadC::checkPassword($_POST['password'], $user['password'])){
					$this->loginUser($user);
				}

				if(!$errors){
					$errors = lang::get("Wrong username or password.");
					jihadC::writeLog('auth bad - wrong username or password');
					sleep(1);
				}
			...
			if (!isset($_GET["login"]) && jihadCconf::get('allow_guests') == true){

				$user = jihadC::getUser('guest');
			...
			jihadC::display("header.php");
			jihadC::display("login.php", array('errors' => $errors));
			jihadC::display("footer.php");
			exit;
		}
	}
	...
	public function loginUser($user){
		...
		jihadC::writeLog('auth ok');

		// reload
		header('Location: '.jihadCconf::get('base_url').'/?cd=');
		die;
	}
	...
	public static function checkPermissions($mode){
		...
		if (jihadCconf::get('use_auth') != true) return true;
	...
	public function getDirectoryList()
	{
		...
		// check if home dir
		if ($directory == jihadCconf::get('repository')) $home = true;
		...
		if ($handler == false){
			$directory = $_SESSION['cwd'] = jihadCconf::get('repository');
			$handler = opendir($directory);
		}

		// open directory and walk through the filenames //HOMEVIDEO
		while (false !== ($file = readdir($handler))) {

			// if file isn't this directory or its parent or resticted, add it to the results
			if ($file != "." && $file != ".." && !in_array($file, jihadCconf::get('restricted_files'))) {

				if (filetype($directory.DS.$file) == 'dir'){

					$link = str_replace(jihadCconf::get('repository'), '', $directory).DS.$file;
					$link = ltrim($link, '/\\');
					...
					if (jihadCconf::get('direct_links_baseurl')) {
						$link = jihadCconf::get('direct_links_baseurl').str_replace(jihadCconf::get('direct_links_basedir'), '', $directory).DS.$file;
					}else{
						$link = jihadCconf::get('base_url').str_replace(jihadCconf::get('base_path'), '', $directory).DS.$file;
					}
		...
		if (!isset($home)){
			...
			$link = str_replace(jihadCconf::get('repository'), '', $parent_directory);
			$link = ltrim($link, '/\\');
		...
		$breadcrumb = array();
		$next = '';
		$bc = 'Home' . str_replace(jihadCconf::get('repository'), '', $directory);
		$bc = explode('/', $bc);
		foreach ($bc as $bclink){
			...
			$breadcrumb[$bclink] = jihadCconf::get('base_url').'/?cd='.$this->encrypt($next);
		}
	...
	public static function error($message){
		...
		echo "<div class=\"warning\"><strong>{$message}</strong></div><a href=\"?logout=1\">".lang::get("Log in")."</a>";

		jihadC::writeLog('error - '.$message);
	...
	public static function validateConf($config, $param){
		...
		if ($param == 'repository'){
			...
			if( isset($_SESSION['simple_auth']['username'])) {
				...
				$user = jihadC::getUser($_SESSION['simple_auth']['username']);
			...
			if (!is_dir($config['repository'])){
				jihadC::error('Directory does not exist: '.$config['repository']);
			}
		}
		...
		if ($param == 'max_filesize'){
			...
			$php_post_max_size =  jihadC::returnBytes(ini_get('post_max_size'));
			$php_upload_max_filesize =  jihadC::returnBytes(ini_get('upload_max_filesize'));

			if ($config['max_filesize'] > $php_post_max_size || $config['max_filesize'] > $php_upload_max_filesize){
				jihadC::error('Config param max_filesize is bigger than php server setting: post_max_size = '.$php_post_max_size.', upload_max_filesize = '.$php_upload_max_filesize);
	...
	public function encrypt($string)
	{
		...
		if (jihadCconf::get('encrypt_url_actions') != true || !isset($_SESSION['simple_auth']['cryptsalt']) || $string == '') return $string;
	...
	public function getDirectoryTree($path, $skip_files = false, $link_prefix = '') {
		...
		foreach($objects as $object){
			...
			// skip unwanted files
			if ($name == '.' || $name == '..' || in_array($name, jihadCconf::get('restricted_files'))) continue;

			$path = str_replace('\\', '/', $object->getPathname());
			$isDir = is_dir($path);

			$link = str_replace(jihadCconf::get('repository'), '', $path);
			$link = jihadC::encodeurl(ltrim($link, '/\\'));
	...
	public static function writeLog($action){
		...
		if (jihadCconf::get('write_log') != true) return;
		...
		file_put_contents(jihadCconf::get('base_path').DS.'usage.log', $log, FILE_APPEND);
	}
	...
	public function sendEmail($to, $subject = '', $body = ''){
		...
		$body .= jihadCconf::get('mail_signature');

		// use simple php mail instead of smtp
		if (jihadCconf::get('use_smtp_protocol') !== true){

			$from = jihadCconf::get('mail_from_name').' <'.jihadCconf::get('mail_from').'>';
			$headers = '';
			$headers .= "From: $from\n";
			...
			jihadC::writeLog('email link sent to: '.$to);
			return;
		}


		require_once jihadCconf::get('base_path')."/include/phpmailer/class.phpmailer.php";

		$mail = new phpmailer(true);

		try {
			$mail->SMTPDebug = jihadCconf::get('mail_smtp_debug');
			$mail->SMTPSecure = jihadCconf::get('mail_smtp_connection_security');
			$mail->IsSMTP(); // send via SMTP
			$mail->Host = jihadCconf::get('mail_smtp_host'); // SMTP servers
			$mail->SMTPAuth = true; // turn on SMTP authentication
			$mail->Username = jihadCconf::get('mail_smtp_username'); // SMTP username
			$mail->Password = jihadCconf::get('mail_smtp_password'); // SMTP password
			$mail->From     = jihadCconf::get('mail_from');
			$mail->FromName = jihadCconf::get('mail_from_name');
			$mail->AddAddress($to);
			...
			if (jihadCconf::get('mail_smtp_debug')) die;
		} catch (Exception $e) {
	...
	public function shortUrl($url){
		...
		$postData = array('longUrl' => $url, 'key' => jihadCconf::get('googl_shorturl_api_key'));
	...
	static function getAllUsers(){
		...
		// use database
		//
		if(jihadCconf::get('use_database')){
		...
		//
		$config_file = jihadCconf::get('user_config_file');
		...
		if (!is_array($_SESSION['cached_users'])){
			jihadC::error('cannot read users file!');
		}
	...
	static function getUser($value, $field = 'username'){
		...
		// use user_config_file
		//
		$current_users = jihadC::getAllUsers();
	...
	static function updateUser($username, $data){
		...
		if(jihadCconf::get('use_database')){
			...
			if (isset($data['homedir'])){
				...
				// if dir does not exist - try to create one
				if ($homedir != '' && !is_dir($homedir)){
					@mkdir($homedir, jihadCconf::get('new_dir_mode'), true);
				}
			}

			if (isset($data['password']) && $data['password'] != ''){
				$password = jihadC::hashPassword($data['password']);
		...
		foreach ($current_users as &$user) {
			...
			if ($user['username'] == $username){
				...
				if (isset($data['password']) && $data['password'] != '') $user['password'] = jihadC::hashPassword($data['password']);
				...
				if (isset($data['homedir'])){
					...
					if ($user['homedir'] != '' && !is_dir($user['homedir'])){
						@mkdir($user['homedir'], jihadCconf::get('new_dir_mode'), true);
					}
				}
				break;
			}
		...
		file_put_contents(jihadCconf::get('user_config_file'), $json_config, LOCK_EX);
	...
	static function addUser($username, $data){
		...
		if(jihadCconf::get('use_database')){
			...
			if (jihadC::getUser($username)) return;
...
class lang {
	public static function get($string) {
		...
		$lang_conf = jihadCconf::get('language');
		$lang_file = jihadCconf::get('base_path')."/languages/{$lang_conf}.php";
