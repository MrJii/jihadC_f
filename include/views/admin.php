...
<?php if(!jihadCconf::get('use_database') && !is_writable(jihadCconf::get('user_config_file'))) echo 'Notice: cannot write to user config file: '.jihadCconf::get('user_config_file');?>

<?php if(jihadCconf::get('use_auth') == true && jihadCconf::get('show_top_auth_bar') == true && $_SESSION['simple_auth']['username'] != 'guest'):?>
<div class="top-menu">
<div class="row">
<a class="version-info"><?php echo lang::get("jCloud")?></a>
...
<div id="logo">
<a href="<?php echo jihadCconf::get('base_url')?>/?cd="><img alt="jCloud" src="<?php echo jihadCconf::get('base_url')?>/include/views/img/logo.gif"></a>
</div>

<button data-username="-1" class="action-account nice radius button" type="button"><?php echo lang::get("Add New User")?></button>
&nbsp;&nbsp;
<a href="<?php echo jihadCconf::get('base_url')?>/?export=csv" class="action-export nice radius secondary button" type="button">Export data</a>

<br /><br />

<div id="browse-panel">

<p>Main Repository: <?php echo jihadCconf::get('repository')?></p>

<table class="file-list">
	<tbody>
	<?php foreach ($params as $user):?>
	<?php if ($user['username'] == 'guest' && !jihadCconf::get('allow_guests')) continue;?>
	<?php if (jihadCconf::get('mask_repository_path')) $user['homedir'] = str_replace(jihadCconf::get('repository'), '', $user['homedir'])?>
		<tr class="accounts" >
			<td class="username">
