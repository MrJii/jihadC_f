...
$(document).ready(function() {
    ...
    $('#fileupload').fileupload('option', {
        url: '<?php echo jihadCconf::get('base_url')?>/?upload=1',
        maxFileSize: <?php echo jihadCconf::get('max_filesize')?>,
        maxNumberOfFiles: <?php echo jihadCconf::get('max_files')?>,
        downloadTemplateId: false,
		autoUpload: true,
        acceptFileTypes: <?php echo jihadCconf::get('accept_file_extensions')?>
    });
    ...
    $('a.version-info').click(function(){
    	...
    	$('#modal').html(' ');
 		...
 		var output = '<div class="modal-content"><h5><?php echo lang::get("jCloud Application")?> <?php echo VERSION?></h5><hr />';
 		output += '<p>Copyright &copy; 2015-<?php echo date('Y')?> by Jii Yo. All right reserved.</p><p>Designed & developed by Jihad Saaduddin. Logo design by SilverBackDavao.</p><p>For more info please visit jCloud App\'s page <a href="http://www.facebook.com/davaocloudstudio" target="_blank">www.jCloudApp.net</a></p>';
    ...
    $('#new-folder').click(function(){
    	$('#newfolder').css('border-color', '#0066cc');
    });
    ...
    $('a.username-edit').click(function(){
		...
		output += '</div>';
 		...
 		$('#confirm-button').click(function(){
			...
			$.post("<?php echo jihadCconf::get('base_url')?>/", { changepassword: password_data} ).done(function(data) {
				// flush 
				window.location.href = '<?php echo jihadCconf::get('base_url')?>/';
			});
    ...
    <?php if (jihadCconf::get('enable_password_recovery') && isset($_SESSION['directlinkenter']) && $_SESSION['directlinkenter'] == 'passwordrecovery'):?>
    ...
    $('a.new-folder').click(function(){
    	...
    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?newdir='+name;
    ...
    $('a.new-file').click(function(){
    	...
    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?newfile='+name;
    ...
    $('.sort-invert').click(function(){
    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?sortinvert';	
    });
    $('.sort-by-name').click(function(){
    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?sortby=name';	
    });
    $('.sort-by-date').click(function(){
    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?sortby=date';	
    });
    $('.sort-by-size').click(function(){
    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?sortby=size';	
    });
    ...
    $('.action-info').click(function(){
    	...
    	var allow_links = '<?php echo jihadCconf::get('allow_file_links')?>';
    	...
    	if (data_type != 'dir'){
    		...
    		// email button
    		<?php if (jihadCconf::get('allow_email_links')):?>
    	...
    	$('#download-button').click(function(){
    		window.location.href = '<?php echo jihadCconf::get('base_url')?>/?download='+data_crypt;
        });
    	...
    	$('#edit-button').click(function(){
    		...
    		$.get('<?php echo jihadCconf::get('base_url')?>/?edit-load='+data_crypt, function(data) {
    ...
    $('.action-account').click(function(){
    	...
    	var main_repository = "<?php if (!jihadCconf::get('mask_repository_path')) echo jihadCconf::get('repository')?>";
    	...
    	$('#save-button').click(function(){
			...
			$.post("<?php echo jihadCconf::get('base_url')?>/?account="+data_username, { is_new: is_new, homedir: homedir, email: email, read: read, write: write, upload: upload, password: set_password} ).done(function(data) {
				// flush 
				window.location.href = '<?php echo jihadCconf::get('base_url')?>/';
			});
    ...
    $('.action-delete-account').click(function(){
	    ...
	    $('#confirm-button').click(function(){
	    	window.location.href = '<?php echo jihadCconf::get('base_url')?>/?account='+data_username+'&delete=yes';
	    });
...
function showDirectoryTree(title, ajaxcall, post_action){
	...
	// get data via ajax
	$.get('<?php echo jihadCconf::get('base_url')?>'+ajaxcall, function(data) {
			$('#dir-links').html(data);
...
function bindTreeLinks(post_action){
	$('#dir-links li[clink]').click(function(e){
		...
		window.location.href = '<?php echo jihadCconf::get('base_url')?>/'+link;
